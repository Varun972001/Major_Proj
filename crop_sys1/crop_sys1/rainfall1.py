...
def weather_readings(lat,lon):
    ...
    one_month_before = one_year_ago - datetime.timedelta(days=75)
    result=weather_fetcher(lat,lon,one_month_before,one_year_ago)
    final_list=store_fvalue(final_list,result)
    print("One Year Ago Pre One Month:"+str(result))
    # print(final_list)

    one_year_ago1 = current_date - datetime.timedelta(days=365)
    one_month_after = one_year_ago1 + datetime.timedelta(days=75)
    result=weather_fetcher(lat,lon,one_year_ago1,one_month_after)
    final_list=store_fvalue(final_list,result)
    print("One Year Ago Post One Month:"+str(result))
    # print(result)
    # print(final_list)

    two_year_ago = current_date - datetime.timedelta(days=731)
    one_month_before1 = two_year_ago - datetime.timedelta(days=75)
    result=weather_fetcher(lat,lon,one_month_before1,two_year_ago)
    final_list=store_fvalue(final_list,result)
    print("Two Year Ago Pre One Month:"+str(result))
    # print(result)
    # print(final_list)

    two_year_ago1 = current_date - datetime.timedelta(days=730)
    one_month_after1 = two_year_ago1 + datetime.timedelta(days=75)
    result=weather_fetcher(lat,lon,two_year_ago1,one_month_after1)
    final_list=store_fvalue(final_list,result)
    print("Two Year Ago Post One Month:"+str(result))
    # print(result)
    print(final_list)
